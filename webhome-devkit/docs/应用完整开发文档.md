...
| minSdk | `24` |
| targetSdk | `28` |
| Android NDK | `28.2.13676358`；仅在重建 MPV/IJK/JNI/DVD native 时使用，普通 Gradle 打包直接复用仓库已提交二进制 |
...
普通 Gradle 打包不编译 MPV、IJK、FFmpeg、OpenSSL、DVD 或 JNI，因而不要求安装 NDK、Perl、`pkg-config` 等 native 工具。只有修改 native 源码、补丁或锁定版本时，才按 3.5 和 3.5.1 节安装 NDK 28.2.13676358；MPV、IJK、JNI 和 DVD 已统一使用这一版 NDK，32 位构建不再需要 NDK 21。
...
| OpenSSL | `openssl-3.2` |
| arm64 NDK | `28.2.13676358`，与 MPV/JNI/DVD 共用 |
| armv7a NDK | `28.2.13676358`，与 MPV/JNI/DVD 共用 |

arm64-v8a 和 armeabi-v7a 均已在 macOS 使用 NDK 28.2.13676358 完整重建成功，并通过 ELF、`SONAME`、`DT_NEEDED` 和未解析标准流符号检查。NDK 28 可以编译 32 位 ARM，旧的 NDK 21.4.7075529 已不再需要。
